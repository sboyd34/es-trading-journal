# Monte Carlo Tab Enhancement Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Extract the Monte Carlo tab into a self-contained `MonteCarloTab` component and add ruin probability, expected value stat card, and max drawdown distribution histogram.

**Architecture:** Create `components/reports/MonteCarloTab.tsx` with all MC state, simulation math, and UI. Expand the simulation loop to track peak equity, per-path max drawdown, and ruin events. Strip the old inline MC code from `reports/page.tsx` and mount the new component in its place, passing `dateFilteredTrades` (fixes a silent bug where the existing sim ignored the date range selector).

**Tech Stack:** React (useState, useMemo), Recharts (BarChart, LineChart, ReferenceLine, Cell), Tailwind, TypeScript
...
|---|---|---|
| Create | `components/reports/MonteCarloTab.tsx` | All MC state, simulation engine, UI |
| Modify | `app/(app)/reports/page.tsx` | Remove inline MC code; mount MonteCarloTab |

---

## Task 1: Create `MonteCarloTab.tsx`

**Files:**
- Create: `components/reports/MonteCarloTab.tsx`

This task creates the new component with the full enhanced simulation engine and all UI. The component is self-contained — it imports nothing from `reports/page.tsx`.

- [ ] **Step 1: Create the file with the complete component**

```tsx
// components/reports/MonteCarloTab.tsx
'use client'
...
export default function MonteCarloTab({ trades, riskRules }: Props) {
  ...
  const monteCarloResult = useMemo((): MonteCarloResult | null => {
    ...
    // Fan chart data (sampled strides for performance)
    ...
    // Max drawdown histogram (numeric midpoints so ReferenceLine aligns)
    ...
    <div className="space-y-6">
      ...
      {mcRunCount === 0 ? (
        ...
        <div className="space-y-6">
          ...
          {/* Final P&L distribution histogram */}
          <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-5">
            <p className="text-sm font-semibold text-gray-300 mb-4">
            ...
            <ResponsiveContainer width="100%" height={200}>
              ...
              >
                ...
                <Tooltip
                  ...
                  formatter={(v: any) => [v, 'Simulations']}
                  labelFormatter={(v) =>
                    `Max DD: ${v >= 1000 ? `$${(Number(v) / 1000).toFixed(1)}k` : `$${Math.round(Number(v))}`}`
                  }
                />
                <Bar dataKey="count" name="Simulations">
...
Expected: no errors. If you see errors referencing `MonteCarloTab.tsx`, fix them before proceeding. Common issues: `formatCurrency` import path wrong, `RiskRules` missing a field.

- [ ] **Step 3: Commit**

```bash
cd ~/es-trading-journal
git add components/reports/MonteCarloTab.tsx
git commit -m "$(cat <<'EOF'
Reports: extract MonteCarloTab with ruin probability and max DD histogram

Self-contained component with enhanced simulation loop: tracks peak equity,
per-path max drawdown, and ruin events. Adds Expected Value and Ruin
Probability stat cards (6 total), plus a Max Drawdown Distribution chart
with a configurable ruin threshold input.
...
## Task 2: Wire `MonteCarloTab` into `reports/page.tsx`

**Files:**
- Modify: `app/(app)/reports/page.tsx`

Four edits to the reports page:
1. Remove `Shuffle` from the lucide-react import (it moves to `MonteCarloTab.tsx`)
2. Add import for `MonteCarloTab`
3. Remove the three inline MC state vars and the `monteCarloResult` useMemo
4. Replace the MC tab JSX block with a single component mount

- [ ] **Step 1: Remove `Shuffle` from lucide imports and add MonteCarloTab import**
...
- [ ] **Step 2: Remove the MC state variables**

Find and remove these two lines (around line 234–235):
```tsx
  const [mcNumTrades, setMcNumTrades] = useState(50)
  const [mcRunCount, setMcRunCount] = useState(0)
```

(The `mcRuinThreshold` state does not exist yet in this file — it lives only in the new component.)

- [ ] **Step 3: Remove the `monteCarloResult` useMemo**

Find and remove this entire block (lines 472–521):
```tsx
...
- [ ] **Step 4: Replace the MC tab JSX block with the component mount**

Find this entire block (lines 1165–1308):
```tsx
...
Expected: no errors. The most likely error at this stage is a leftover reference to `mcRunCount`, `mcNumTrades`, or `monteCarloResult` — search the file for any remaining references and remove them.

```bash
grep -n "mcRunCount\|mcNumTrades\|monteCarloResult" ~/es-trading-journal/app/\(app\)/reports/page.tsx
```

Expected: no output. If you see matches, remove those lines.
...
git commit -m "$(cat <<'EOF'
Reports: mount MonteCarloTab, remove inline MC code

Replaces 144 lines of inline MC state/useMemo/JSX with a single component
mount. Passes dateFilteredTrades (fixes bug: inline sim ignored date range
selector). Removes Shuffle from lucide imports in this file.
...
- [ ] **Step 2: Verify the Monte Carlo tab loads**

Navigate to **Reports → Monte Carlo** tab. You should see:
- Header: "Monte Carlo Simulator"
- Two inputs: "Trades per path" and "Ruin threshold ($)"
- "Run Simulation" button
- Dashed empty state panel

- [ ] **Step 3: Verify ruin threshold pre-fills**

The "Ruin threshold" input should show your `max_daily_loss` value from Risk Rules settings. If you have no risk rules set, it should show `1500`.

- [ ] **Step 4: Run a simulation and check all outputs**

Click "Run Simulation". After it completes, verify:

| Element | Expected |
|---|---|
| Stat card count | 6 cards in a 2→3→6 responsive grid |
| "Expected Value" card | Visible, dollar amount, green or red |
| "Ruin Probability" card | Visible, percentage, color-coded (green/amber/red) |
| Equity path fan chart | Unchanged — 5 percentile lines with legend |
| Final P&L histogram | Unchanged — green/red bars |
| Max Drawdown histogram | New — gray/red bars, "Ruin" label line visible |

- [ ] **Step 5: Verify date range filter applies to MC**

Set date range to "This Week" (top of Reports page). Click "Re-Run". The caption in the Final P&L histogram should show a lower trade count than "All Time" if you have trades outside this week.

- [ ] **Step 6: Verify ruin threshold changes trigger re-run correctly**

Change the "Ruin threshold" input to a different value. The simulation results should NOT auto-recompute (the useMemo only fires on `mcRunCount` increment, `mcNumTrades`, `trades`, and `mcRuinThreshold` — the last of which does change, so the max DD histogram bars will update immediately without clicking Re-Run). Confirm the max DD histogram and ruin probability update when you tab out of the threshold input.
